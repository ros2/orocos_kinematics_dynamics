...
class KinfamTestFunctions(unittest.TestCase):
    def setUp(self):
        self.chain = Chain()
        self.chain.addSegment(Segment(Joint(Joint.RotZ), Frame(Vector(0.0, 0.0, 0.0))))
        self.chain.addSegment(Segment(Joint(Joint.RotX), Frame(Vector(0.0, 0.0, 0.9))))
        self.chain.addSegment(
            Segment(Joint(getattr(Joint, "None")), Frame(Vector(-0.4, 0.0, 0.0)))
        )
        self.chain.addSegment(Segment(Joint(Joint.RotY), Frame(Vector(0.0, 0.0, 1.2))))
        self.chain.addSegment(
            Segment(Joint(getattr(Joint, "None")), Frame(Vector(0.4, 0.0, 0.0)))
        )
        self.chain.addSegment(
            Segment(Joint(Joint.TransZ), Frame(Vector(0.0, 0.0, 1.4)))
        )
        self.chain.addSegment(
            Segment(Joint(Joint.TransX), Frame(Vector(0.0, 0.0, 0.0)))
        )
        self.chain.addSegment(
            Segment(Joint(Joint.TransY), Frame(Vector(0.0, 0.0, 0.4)))
        )
        self.chain.addSegment(
            Segment(Joint(getattr(Joint, "None")), Frame(Vector(0.0, 0.0, 0.0)))
        )
        ...
        self.iksolverpos = ChainIkSolverPos_NR(
            self.chain, self.fksolverpos, self.iksolvervel
        )
    ...
    def testFkPosAndJac(self):
        ...
        for i in range(q.rows()):
            ...
            Jcol2 = Twist(
                Vector(jac[0, i], jac[1, i], jac[2, i]),
    ...
    def testFkVelAndIkVel(self):
        ...
        self.assertTrue(
            0 == self.iksolvervel.CartToJnt(qvel.q, cart.deriv(), qdot_solved)
        )
...
class KinfamTestTree(unittest.TestCase):
    def setUp(self):
        self.tree = Tree()
        self.tree.addSegment(
            Segment(Joint(Joint.RotZ), Frame(Vector(0.0, 0.0, 0.0))), "foo"
        )
        self.tree.addSegment(
            Segment(Joint(getattr(Joint, "None")), Frame(Vector(0.0, 0.0, 0.0))), "bar"
        )
